...
from handlers.channel import user_in_channels, build_join_keyboard
...
async def deliver_callback_handler(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if query.data.startswith("check_"):
        ...
        is_member = await user_in_channels(context, query.from_user.id)
        if not is_member:
            await query.answer(
                "❌ Pehle join request bhejo channel mein, phir dobara try karo!",
                show_alert=True,
            )
            return
